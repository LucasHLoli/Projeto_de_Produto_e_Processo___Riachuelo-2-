...
\phantomsection
\section*{ANEXO A \textendash{} ROTEIRO DE ENTREVISTA INDIVIDUAL SEMIESTRUTURADA}
\label{anexo:roteiro_entrevistas}
\addcontentsline{toc}{section}{ANEXO A \textendash{} ROTEIRO DE ENTREVISTA INDIVIDUAL SEMIESTRUTURADA}

O roteiro a seguir foi o instrumento utilizado nas entrevistas individuais semiestruturadas descritas na Seção~\ref{subsec:entrevistas}. As perguntas estão na ordem em que foram feitas, numa sequência única, sem divisão em blocos temáticos. As perguntas de aprofundamento (\textit{probes}) aparecem em itálico, logo após a pergunta principal a que se referem, e o entrevistador as usava apenas quando faziam sentido na conversa. Os trechos entre colchetes são instruções de condução, que não eram lidas em voz alta.
...
\begin{enumerate}
    ...
    \item\relax [Retomar, sem repetir de forma idêntica, a pergunta anterior sobre o que o entrevistado mudaria no processo discutido.]
    \item\relax [Descrever o conceito de forma neutra:] ``a Riachuelo está estudando uma etiqueta de segurança na peça que passa a se comunicar com o aplicativo da loja. Ela sabe se a peça foi paga e, quando o pagamento é confirmado pelo aplicativo, ela se destrava sozinha, sem precisar passar pelo caixa.''
    \item\relax [Perguntar a reação livre do entrevistado antes de detalhar qualquer aspecto técnico ou de segurança. Só depois, e apenas se o entrevistado não tocar no assunto espontaneamente, seguir com as perguntas de confiança e segurança abaixo.]
